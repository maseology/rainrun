...
// TriangularTF similar to the HBV MAXBAS transfer function with the option of skewing the mode
...
func TriangularTF(base, skew, lag float64) []float64 {
	if base < 0. || lag < 0. || fracCheck(skew) {
		panic("TriangularTF input error")
	}
